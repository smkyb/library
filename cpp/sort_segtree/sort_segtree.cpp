...
struct sort_segtree {
    ...
    struct binary_trie{
        ...
        struct node_t{
            ...
            node_t() = default;
            node_t(T v, SS s, int w, int c) : value(v), sum(s), width(w), count(c), child{&nil, &nil, &nil, &nil} {}
            node_t(T v, SS s, int w, int c, node_t* c0, node_t* c1, node_t* c2, node_t* c3) : value(v), sum(s), width(w), count(c), child{c0, c1, c2, c3} {}
            static node_t nil;
        };
        ...
        binary_trie(){
            root = make_node(0, SS::SS_e(), bit_width, 0);
        }
        binary_trie(T p, const S &x){
        ...
        void insert(T v, const SS &x) {
            ...
            for(int i = route_cnt-1; i >= 0; i--){
                node_t *ptr = route[i];
                ptr->sum = SS::SS_op(SS::SS_op(SS::SS_op(ptr->child[0]->sum, ptr->child[1]->sum), ptr->child[2]->sum), ptr->child[3]->sum);
            }
        }
        ...
        static node_t *meld(node_t *l, node_t *r, int bit){
            ...
            l->sum = SS::SS_op(SS::SS_op(SS::SS_op(l->child[0]->sum, l->child[1]->sum), l->child[2]->sum), l->child[3]->sum);
            return l;
        }
        ...
        static pair<node_t*, node_t*> split(node_t *l, int p){
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (p < acc + c) {
                    ...
                    node_t *ptr = make_node(l->value, SS::SS_e(), l->width, 0);
                    ...
                    l->sum = SS::SS_e();
                    for(int j = 0; j <= i; j++) l->sum = SS::SS_op(l->sum, l->child[j]->sum);
                    for(int j = i; j < 4; j++) ptr->sum = SS::SS_op(ptr->sum, ptr->child[j]->sum);
        ...
        node_t *split_one(node_t *l){
            ...
            for(int i = 0; i < 4; i++){
                ...
                l->sum = SS::SS_e();
                for(int j = i; j < 4; j++) l->sum = SS::SS_op(l->sum, l->child[j]->sum);
                return l;
            }
            assert(false);
        }
        ...
        SS all_prod() const {return root->sum;}
...
template<typename T, typename S, auto op, auto e> typename sort_segtree<T, S, op, e>::binary_trie::node_t sort_segtree<T, S, op, e>::binary_trie::node_t::nil = node_t(0, SS::SS_e(), bit_width, 0, &node_t::nil, &node_t::nil, &node_t::nil, &node_t::nil);
template<typename T, typename S, auto op, auto e> typename sort_segtree<T, S, op, e>::binary_trie::Pool sort_segtree<T, S, op, e>::binary_trie::pool = Pool();
